...
As objects usually do not deform so much, they are generally with relatively fixed shape in some extent, which infers that the target bounding box will have certain relationship with aspect ratio especially for non-living objects. An intuitive example is objects of bottle class will be more likely to have large aspect ratios (width to height), i.e. tall and thin. According to the statistics, both of the two commonly used datasets, VOC and MS COCO, are showing the regulation. Following histograms describe that for some classes objects are tend to be with some aspect ratios. 
...
Faster R-CNN network consists of three parts: feature extractor, region proposal network(RPN) and head(classifier and regressor). Our idea is to combine aspect ratio with RPN. Original RPN in faster R-CNN used 9 anchor boxes with three kinds of aspect ratio(1:2,1:1,2:1)\cite{fasterRCNN} and and three scales (0.5,1,2). There are some papers study on RPN based on multiscale encouraging us to study on splitting RPN into three RPNs based on aspect ratio make the RPN aspect ratios sensitive. The following pies show the distribution of datasets when dividing the aspect ratios into three parts. (0-0.75: 1:2； 0.75-1.5：1：1；1.5-？ 2：1)
