...
Most traditional region proposal methods are based on low-level features. Faster-RCNN introduced Region Proposal Network (RPN) to generate region proposals based on high-level semantic features. RPN supass conventional region proposal methods on both speed and accuracy, either unsupervised conventional method (e.g. Selective Search~\cite{SelectiveSearch}) or supervised conventional method (e.g. BING~\cite{BING}).
